...
api.getAPI = function(namespace, apiName, path, prefix, options){
    if(global['EXT_REMOTING_API']){
        return EXT_REMOTING_API.api;
    }else{
        return createCachedAPI(namespace, apiName, path, prefix, options);
    }
};

function createCachedAPI(namespace, apiName, path, prefix, options){
    ...
    // Merge/Replace options properties into res
    ...
    if (options) {
        ...
        for (var attrname in options) {
            ...
            if (options.hasOwnProperty(attrname)) {
                console.log('copying attribute' + attrname)
